...
```{r}
names(Added_value)[names(Added_value) == "OBS_VALUE"] <- "Added_Value"
names(GDP)[names(GDP) == "OBS_VALUE"] <- "GDP"
names(Envt_Tax_Revenues)[names(Envt_Tax_Revenues) == "OBS_VALUE"] <- "Envt_Tax_Revenues"
names(GHG_Emissions)[names(GHG_Emissions) == "OBS_VALUE"] <- "GHG_Emissions"
names(Warm_Households)[names(Warm_Households) == "OBS_VALUE"] <- "Warm_Households"
```
...
ggplot(EU27_Tax, aes(x = TIME_PERIOD, y = OBS_VALUE, color = CATEGORY, group = CATEGORY)) +
  geom_line(size = 1) +
  labs(
